...
export async function configureBbyCredential(env:BbyEnv,store:Store,apiKey:string){await ensureBbySchema(env.DB,store);if(!apiKey.trim())throw new Error("Mirakl API key is required");const t=tables(store),iv=crypto.getRandomValues(new Uint8Array(12));const ciphertext=await crypto.subtle.encrypt({name:"AES-GCM",iv},await encryptionKey(env),new TextEncoder().encode(apiKey.trim()));await env.DB.prepare(`INSERT INTO ${t.credentials}(id,ciphertext,iv,updated_at) VALUES(1,?,?,?) ON CONFLICT(id) DO UPDATE SET ciphertext=excluded.ciphertext,iv=excluded.iv,updated_at=excluded.updated_at`).bind(base64(new Uint8Array(ciphertext)),base64(iv),Date.now()).run();return {store,configured:true};}
async function credential(env: BbyEnv, store: Store) {const t=tables(store);const saved=await env.DB.prepare(`SELECT ciphertext,iv FROM ${t.credentials} WHERE id=1`).first<{ciphertext:string;iv:string}>();if(saved){const plaintext=await crypto.subtle.decrypt({name:"AES-GCM",iv:bytes(saved.iv)},await encryptionKey(env),bytes(saved.ciphertext));return new TextDecoder().decode(plaintext);}const apiKey = store === "JS" ? env.BESTBUY_JS_API_KEY : env.BESTBUY_SAP_API_KEY; if (!apiKey) throw new Error(`Best Buy ${store} Mirakl secret is not configured`); return apiKey; }
...
async function syncBbyReturnReasons(env:BbyEnv,store:Store,from:string,to:string){
  ...
  await runBatches(env.DB,catalogRows.map(reason=>env.DB.prepare(`INSERT OR REPLACE INTO ${t.reasonCatalog}(code,label,reason_type,updated_at) VALUES(?,?,?,?)`).bind(s(reason.code),s(reason.label,reason.code),s(reason.type),now)));
  const labels=await env.DB.prepare(`SELECT code,label FROM ${t.reasonCatalog}`).all<{code:string;label:string}>(),labelMap=new Map(labels.results.map(row=>[row.code,row.label]));
...
export async function syncBby(env: BbyEnv, store: Store, from: string, to: string) {
  await ensureBbySchema(env.DB,store); const t=tables(store),windowId=`${from}:${to}`; const existing=await env.DB.prepare(`SELECT status FROM ${t.windows} WHERE id=?`).bind(windowId).first<{status:string}>(); if(existing?.status==="success"){const reasons=await syncBbyReturnReasons(env,store,from,to);return {store,orders:0,returns:0,reasons,skipped:true};} let offset=0,ordersCount=0,returnsCount=0;
  for(let page=0;page<500;page++) { const data=await mirakl(env,store,"/api/orders",{start_date:`${from}T00:00:00Z`,end_date:`${to}T23:59:59Z`,max:"100",offset:String(offset),order_tax_mode:"TAX_INCLUDED"}); const orders=(data.orders||[]) as Obj[];
    for(const order of orders){ const orderId=s(order.order_id,order.id); const orderedAt=ts(order.created_date,order.date_created); for(const line of (order.order_lines||[]) as Obj[]){ const lineId=s(line.order_line_id,line.id); const sku=s(line.offer_sku,line.product_sku,line.product?.sku,"UNKNOWN-SKU"); const quantity=Math.max(1,n(line.quantity)||1); const gross=n(line.price)||n(line.unit_price)*quantity; await env.DB.prepare(`INSERT INTO ${t.sales} VALUES(?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET sku=excluded.sku,product_name=excluded.product_name,quantity=excluded.quantity,gross_sales=excluded.gross_sales,status=excluded.status,raw_json=excluded.raw_json,updated_at=excluded.updated_at`).bind(`${store}:${lineId}`,orderId,sku,s(line.product_title,line.product?.title,sku),quantity,orderedAt,gross,s(line.order_line_state,order.order_state),JSON.stringify({order,line}),Date.now()).run(); ordersCount++;
      for(const refund of (line.refunds||[]) as Obj[]){ const id=s(refund.refund_id,refund.id,`${lineId}:${refund.created_date||refund.date_created||refund.reason_code}`); const qty=Math.max(1,n(refund.quantity)||1); await env.DB.prepare(`INSERT INTO ${t.returns} VALUES(?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET reason=excluded.reason,quantity=excluded.quantity,refund_amount=excluded.refund_amount,status=excluded.status,raw_json=excluded.raw_json,updated_at=excluded.updated_at`).bind(`${store}:${id}`,orderId,lineId,sku,s(refund.reason_label,refund.reason_code,"Refund"),qty,n(refund.amount)||n(refund.refund_amount),s(refund.state,refund.status,"REFUNDED"),ts(refund.created_date,refund.date_created,order.last_updated_date),JSON.stringify(refund),Date.now()).run(); returnsCount++; }
    }} if(orders.length<100) break; offset+=orders.length;
  }
  const reasons=await syncBbyReturnReasons(env,store,from,to);await env.DB.prepare(`INSERT INTO ${t.windows} VALUES(?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET status='success',orders_count=excluded.orders_count,returns_count=excluded.returns_count,updated_at=excluded.updated_at`).bind(windowId,from,to,"success",ordersCount,returnsCount,Date.now()).run();
...
export async function getBbyReturns(db:D1Database, input:{store:string;from:string;to:string;granularity:Granularity;sku?:string}) {
  ...
  const [salesResult,returnsResult,createdResult,excludedResult]=await Promise.all([
    db.prepare(`SELECT order_id,sku,product_name,quantity,ordered_at,gross_sales,status FROM ${t.sales} WHERE ordered_at BETWEEN ? AND ?`).bind(start,end).all<any>(),
    db.prepare(`SELECT r.order_id,r.line_item_id,r.sku,COALESCE((SELECT rr.reason_label FROM ${t.returnReasons} rr WHERE rr.line_item_id=r.line_item_id AND (rr.order_id=r.order_id OR rr.order_id='') ORDER BY rr.requested_at DESC LIMIT 1),r.reason) AS reason,CASE WHEN EXISTS(SELECT 1 FROM ${t.returnReasons} rr WHERE rr.line_item_id=r.line_item_id AND (rr.order_id=r.order_id OR rr.order_id='')) THEN 1 ELSE 0 END AS reason_mapped,r.quantity,r.refund_amount,r.status,r.requested_at FROM ${t.returns} r WHERE EXISTS (SELECT 1 FROM ${t.sales} s WHERE s.order_id=r.order_id AND s.sku=r.sku AND s.ordered_at BETWEEN ? AND ?)`).bind(start,end).all<any>(),
    db.prepare(`SELECT COALESCE(SUM(quantity),0) AS count FROM ${t.returns} WHERE requested_at BETWEEN ? AND ?`).bind(start,end).first<{count:number}>(),
    db.prepare(`SELECT order_id AS orderId FROM ${t.exclusions}`).all<{orderId:string}>(),
  ]);
  ...
  const bySku=new Map<string,any>(); for(const sale of commercialSales){if(input.sku&&input.sku!=="ALL"&&sale.sku!==input.sku)continue; const row=bySku.get(sale.sku)||{sku:sale.sku,productName:sale.product_name,soldUnits:0,returnedUnits:0,refundAmount:0,reasons:new Map(),trend:new Map()}; row.soldUnits+=sale.quantity; const k=key(sale.ordered_at,input.granularity); const bucket=row.trend.get(k)||{key:k,soldUnits:0,returnedUnits:0,returnRate:0};bucket.soldUnits+=sale.quantity;row.trend.set(k,bucket);bySku.set(sale.sku,row);}
  for(const ret of returns){if(excludedOrders.has(ret.order_id))continue;const row=bySku.get(ret.sku);if(!row)continue; row.returnedUnits+=ret.quantity;row.refundAmount+=ret.refund_amount;row.reasons.set(ret.reason,(row.reasons.get(ret.reason)||0)+ret.quantity); const sale=commercialSales.find(x=>x.order_id===ret.order_id&&x.sku===ret.sku);if(sale){const bucket=row.trend.get(key(sale.ordered_at,input.granularity));if(bucket)bucket.returnedUnits+=ret.quantity;}}
