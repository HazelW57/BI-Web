...
function tables(store:Store){const prefix=store==="JS"?"bby_js":"bby_sap";return{sales:`${prefix}_sales_lines`,returns:`${prefix}_return_lines`,windows:`${prefix}_sync_windows`,credentials:`${prefix}_credentials`,exclusions:`${prefix}_excluded_orders`,prefix};}
...
export async function syncBby(env: BbyEnv, store: Store, from: string, to: string) {
  await ensureBbySchema(env.DB,store); const t=tables(store),windowId=`${from}:${to}`; const existing=await env.DB.prepare(`SELECT status FROM ${t.windows} WHERE id=?`).bind(windowId).first<{status:string}>(); if(existing?.status==="success")return {store,orders:0,returns:0,skipped:true}; let offset=0,ordersCount=0,returnsCount=0;
  ...
  await env.DB.prepare(`INSERT INTO ${t.windows} VALUES(?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET status='success',orders_count=excluded.orders_count,returns_count=excluded.returns_count,updated_at=excluded.updated_at`).bind(windowId,from,to,"success",ordersCount,returnsCount,Date.now()).run();
...
export async function getBbyReturns(db:D1Database, input:{store:string;from:string;to:string;granularity:Granularity;sku?:string}) {
  ...
  const [salesResult,returnsResult,createdResult,excludedResult]=await Promise.all([
    ...
    db.prepare(`SELECT r.order_id,r.sku,r.reason,r.quantity,r.refund_amount,r.status,r.requested_at FROM ${t.returns} r WHERE EXISTS (SELECT 1 FROM ${t.sales} s WHERE s.order_id=r.order_id AND s.sku=r.sku AND s.ordered_at BETWEEN ? AND ?)`).bind(start,end).all<any>(),
