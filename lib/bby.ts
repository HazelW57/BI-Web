...
export async function syncBby(env: BbyEnv, store: Store, from: string, to: string) {
  await ensureBbySchema(bbyDatabase(env,store),store); const t=tables(store),windowId=`${from}:${to}`; const existing=await bbyDatabase(env,store).prepare(`SELECT status FROM ${t.windows} WHERE id=?`).bind(windowId).first<{status:string}>(); if(existing?.status==="success"){const reasons=await syncBbyReturnReasons(env,store,from,to);return {store,orders:0,returns:0,reasons,skipped:true};} let offset=0,ordersCount=0,returnsCount=0;
  for(let page=0;page<500;page++) { const data=await mirakl(env,store,"/api/orders",{start_date:`${from}T00:00:00Z`,end_date:`${to}T23:59:59Z`,max:"100",offset:String(offset),order_tax_mode:"TAX_INCLUDED"}); const orders=(data.orders||[]) as Obj[];
    for(const order of orders){ const orderId=s(order.order_id,order.id); const orderedAt=ts(order.created_date,order.date_created); for(const line of (order.order_lines||[]) as Obj[]){ const lineId=s(line.order_line_id,line.id); const sku=s(line.offer_sku,line.product_sku,line.product?.sku,"UNKNOWN-SKU"); const quantity=Math.max(1,n(line.quantity)||1); const gross=n(line.price)||n(line.unit_price)*quantity; await bbyDatabase(env,store).prepare(`INSERT INTO ${t.sales} VALUES(?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET sku=excluded.sku,product_name=excluded.product_name,quantity=excluded.quantity,gross_sales=excluded.gross_sales,status=excluded.status,raw_json=excluded.raw_json,updated_at=excluded.updated_at`).bind(`${store}:${lineId}`,orderId,sku,s(line.product_title,line.product?.title,sku),quantity,orderedAt,gross,s(line.order_line_state,order.order_state),JSON.stringify({order,line}),Date.now()).run(); ordersCount++;
...
export async function getBbyReturns(db:D1Database, input:{store:string;from:string;to:string;granularity:Granularity;sku?:string}) {
  ...
  const [salesResult,returnsResult,createdResult,excludedResult,coverageResult]=await Promise.all([
    ...
    db.prepare(`SELECT MAX(ordered_at) AS latest FROM ${t.sales}`).first<{latest:number|null}>(),
  ]);
