...
type PnlData = { range: { from: string; to: string }; dataThrough?: string | null; granularity: string; dimensions: Dimensions; total: PnlRow; trend: PnlRow[]; months: PnlRow[]; skus: PnlRow[]; sources: Source[]; financeCoverage?: { mappedLines: number; totalLines: number; percent: number; statementCount: number; settlementSummary: boolean; status: string }; affiliateCoverage?: { attributedOrders:number; mappedRows:number; totalRows:number; finalRows:number; estimatedRows:number; finalAmount:number; estimatedAmount:number; percent:number; status:string }; orderMix?: { commercialOrders:number; sampleOrders:number; returnedOrders:number; commercialUnits:number; sampleUnits:number } };
...
type ReturnSku = {
  ...
  reasonAudit?: { workflowUnits: number; duplicateUnits: number; unavailableReasonUnits: number };
};
type ReturnsData = {
...
type SyncData = {
  ...
  counts?: { salesLines: number; returnLines: number; productCostRules: number; agencyRules: number; returnShippingRules: number; manualCosts: number; financeOrders: number; validOrders: number; affiliateAttributions:number; affiliateMapped:number; affiliateFinal:number; affiliateEstimated:number };
};
...
export default function Dashboard({ email, admin, initialAllowed }: { email: string; admin: boolean; initialAllowed: Allowed[] }) {
  ...
  const [maxDate, setMaxDate] = useState(dateInput(today));
  ...
  const [walmartReturns,setWalmartReturns]=useState<ReturnsData>(emptyReturns);
  const [walmartBusy,setWalmartBusy]=useState(false);
  const [walmartError,setWalmartError]=useState("");
  const [walmartNotice,setWalmartNotice]=useState("");
  const [walmartConfigOpen,setWalmartConfigOpen]=useState(false);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      // Keep D1 traffic bounded while channel backfills are running. Parallel
      // dashboard requests can otherwise exhaust the Worker/D1 connection
      // budget even though each endpoint is healthy in isolation.
  ...
  }, [from, getJson, granularity, product, returnStatus, returnType, sku, tab, to]);
  ...
  const refreshBby = useCallback(async () => {
    ...
    try { const query=new URLSearchParams({store:bbyStore,from,to,granularity,sku}); const data=await getJson<ReturnsData>(`/api/bby/returns?${query}`,{signal:controller.signal}); if(requestId===bbyRequestId.current){setBbyReturns(data);if(data.dataThrough){setMaxDate(data.dataThrough);if(to>data.dataThrough)setTo(data.dataThrough);}} }
  ...
  const refreshWalmart=useCallback(async()=>{setWalmartBusy(true);setWalmartError("");try{const q=new URLSearchParams({from,to,granularity,sku});const data=await getJson<ReturnsData>(`/api/walmart/returns?${q}`);setWalmartReturns(data);if(data.dataThrough){setMaxDate(data.dataThrough);if(to>data.dataThrough)setTo(data.dataThrough);}}catch(cause){setWalmartError(cause instanceof Error?cause.message:"Walmart data unavailable");}finally{setWalmartBusy(false);}},[from,to,granularity,sku,getJson]);
  useEffect(()=>{if(tab!=="walmart")return;const timer=window.setTimeout(()=>void refreshWalmart(),80);return()=>window.clearTimeout(timer);},[tab,refreshWalmart]);
  async function configureWalmartNow(){if(!walmartClientId.trim()||!walmartClientSecret.trim())return;setWalmartBusy(true);setWalmartError("");try{await getJson("/api/walmart/credential",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({clientId:walmartClientId.trim(),clientSecret:walmartClientSecret.trim()})});setWalmartNotice("Walmart API credentials encrypted and saved.");setWalmartClientId("");setWalmartClientSecret("");setWalmartConfigOpen(false);}catch(cause){setWalmartError(cause instanceof Error?cause.message:"Configuration failed");}finally{setWalmartBusy(false);}}
  async function syncWalmartNow(){setWalmartBusy(true);setWalmartError("");try{let cursor=new Date("2025-10-01T00:00:00Z"),end=new Date(),orders=0,returned=0;while(cursor<=end){const a=dateInput(cursor),weekEnd=new Date(cursor.getTime()+6*86_400_000),b=dateInput(weekEnd>end?end:weekEnd);setWalmartNotice(`Walmart syncing ${a} — ${b}`);const result=await getJson<{orders:number;returns:number}>("/api/walmart/sync",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({from:a,to:b})});orders+=result.orders;returned+=result.returns;cursor=new Date(cursor.getTime()+7*86_400_000);}setWalmartNotice(`Walmart sync complete: ${orders} order lines, ${returned} return lines.`);await refreshWalmart();}catch(cause){setWalmartError(cause instanceof Error?cause.message:"Walmart sync failed");}finally{setWalmartBusy(false);}}
  ...
  async function syncBbyNow(){setBbyBusy(true);setBbyError("");try{const latest=bbyReturns.dataThrough||"2025-09-30";let cursor=new Date(`${latest}T00:00:00Z`),end=new Date();cursor.setUTCDate(cursor.getUTCDate()+1);let orders=0,returned=0,windows=0;while(cursor<=end){const a=dateInput(cursor),weekEnd=new Date(cursor.getTime()+6*86_400_000),b=dateInput(weekEnd>end?end:weekEnd);setBbyNotice(`Best Buy ${bbyStore} 同步中：${a} — ${b}（每周安全窗口）`);let result:{orders:number;returns:number}|undefined;for(let attempt=1;attempt<=4;attempt++){try{result=await getJson<{orders:number;returns:number}>("/api/bby/sync",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({store:bbyStore,from:a,to:b})});break;}catch(cause){if(!(cause instanceof Error)||!cause.message.includes("429")||attempt===4)throw cause;setBbyNotice(`Mirakl rate limit：${a} 将在 ${attempt*3} 秒后重试`);await new Promise(resolve=>setTimeout(resolve,attempt*3000));}}orders+=result?.orders||0;returned+=result?.returns||0;windows++;cursor.setUTCDate(cursor.getUTCDate()+7);}setBbyNotice(`Best Buy ${bbyStore} 同步完成：${windows} 个窗口，${orders} 条销售行，${returned} 条退款行。`);await refreshBby();}catch(cause){setBbyError(cause instanceof Error?cause.message:"Best Buy 同步失败");setBbyBusy(false);}}
  async function backfillBbyReasons(){setBbyBusy(true);setBbyError("");try{let cursor=new Date(`${from}T00:00:00Z`),end=new Date(`${to}T00:00:00Z`),mapped=0,pages=0,windows=0;while(cursor<=end){const start=dateInput(cursor),monthEnd=new Date(Date.UTC(cursor.getUTCFullYear(),cursor.getUTCMonth()+1,0)),windowEnd=dateInput(monthEnd>end?end:monthEnd);setBbyNotice(`Best Buy ${bbyStore} Return Reason 回填：${start} — ${windowEnd}`);const result=await getJson<{mapped:number;pages:number}>("/api/bby/return-reasons",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({store:bbyStore,from:start,to:windowEnd})});mapped+=result.mapped;pages+=result.pages;windows++;cursor=new Date(Date.UTC(cursor.getUTCFullYear(),cursor.getUTCMonth()+1,1));}setBbyNotice(`Best Buy ${bbyStore} Return Reason 完成：${windows} 个月，${pages} 页，${mapped} 条平台原因。`);await refreshBby();}catch(cause){setBbyError(cause instanceof Error?cause.message:"Return Reason 回填失败");}finally{setBbyBusy(false);}}
  ...
  async function backfillAffiliateNow() {
    ...
    try {
      ...
      while (cursor < end) {
        ...
        const result = await getJson<{ attributions: number; financeRowsFinal: number; financeOrdersPending: number }>("/api/bi/sync", {
          method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ mode: "affiliate", from, to }),
        });
  ...
  return <main className="shell">
    ...
    <section className="workspace">
      <header className="topbar"><div><p className="crumb">SAPHIANT / COMMERCE INTELLIGENCE</p><h1>{nav.find((item) => item.key === tab)?.label}</h1></div><div className="top-status">{tab==="tts"?<span className={sync.configured ? "live" : "pending"}>{sync.configured ? "TTS 数据在线" : "TTS API 待配置"}</span>:tab==="bby"?<span className={bbyBusy?"pending":"live"}>{bbyBusy?`BBY ${bbyStore} 同步中`:`BBY ${bbyStore} 独立数据库`}</span>:tab==="walmart"?<span className={walmartBusy?"pending":"live"}>{walmartBusy?"Walmart 同步中":"Walmart 独立数据库"}</span>:<span className="live">SAPHIANT BI</span>}</div></header>
      ...
      {tab === "bby" && <><div className="subnav tts-subnav"><button disabled>P&L · Next</button><button className={bbyView==="returns"?"active":""} onClick={()=>setBbyView("returns")}>R&R</button><button className={bbyView==="exclusions"?"active":""} onClick={()=>setBbyView("exclusions")}>刷单提交</button><button onClick={()=>void syncBbyNow()} disabled={bbyBusy||!admin}>{bbyBusy?"Syncing…":`Sync ${bbyStore} Mirakl`}</button><button onClick={()=>void backfillBbyReasons()} disabled={bbyBusy||!admin}>Backfill Return Reasons</button></div>{bbyView==="returns"&&<GlobalFilters maxDate={maxDate} from={from} to={to} setFrom={setFrom} setTo={setTo} granularity={granularity} setGranularity={setGranularity} product={product} setProduct={setProduct} sku={sku} setSku={setSku} dimensions={bbyReturns.dimensions} returnsMode={false} returnType={returnType} setReturnType={setReturnType} returnStatus={returnStatus} setReturnStatus={setReturnStatus} busy={bbyBusy} refresh={refreshBby}/>}</>}
      {tab === "walmart" && <><div className="subnav tts-subnav"><button disabled>P&L · Next</button><button className="active">R&R</button>{admin&&<button onClick={()=>setWalmartConfigOpen(value=>!value)} disabled={walmartBusy}>Configure API</button>}<button onClick={()=>void syncWalmartNow()} disabled={walmartBusy||!admin}>{walmartBusy?"Syncing…":"Sync Walmart"}</button></div>{admin&&walmartConfigOpen&&<div className="filters walmart-config"><label>Client ID<input aria-label="Walmart Client ID" value={walmartClientId} onChange={event=>setWalmartClientId(event.target.value)} autoComplete="off"/></label><label>Client Secret<input aria-label="Walmart Client Secret" type="password" value={walmartClientSecret} onChange={event=>setWalmartClientSecret(event.target.value)} autoComplete="new-password"/></label><button onClick={()=>void configureWalmartNow()} disabled={walmartBusy||!walmartClientId.trim()||!walmartClientSecret.trim()}>Save API</button><button className="ghost" onClick={()=>{setWalmartConfigOpen(false);setWalmartClientId("");setWalmartClientSecret("");}}>Cancel</button></div>}<GlobalFilters maxDate={maxDate} from={from} to={to} setFrom={setFrom} setTo={setTo} granularity={granularity} setGranularity={setGranularity} product={product} setProduct={setProduct} sku={sku} setSku={setSku} dimensions={walmartReturns.dimensions} returnsMode={false} returnType={returnType} setReturnType={setReturnType} returnStatus={returnStatus} setReturnStatus={setReturnStatus} busy={walmartBusy} refresh={refreshWalmart}/></>}
      ...
      {tab === "tts" && ttsTab !== "health" && ttsTab !== "costs" && <GlobalFilters maxDate={maxDate} from={from} to={to} setFrom={setFrom} setTo={setTo} granularity={granularity} setGranularity={setGranularity} product={product} setProduct={setProduct} sku={sku} setSku={setSku} dimensions={dimensions} returnsMode={ttsTab === "returns"} returnType={returnType} setReturnType={setReturnType} returnStatus={returnStatus} setReturnStatus={setReturnStatus} busy={busy} refresh={refresh} />}
      ...
      {tab==="walmart" && walmartError && <div className="message error-message">{walmartError}<button onClick={()=>setWalmartError("")}>×</button></div>}
      {tab==="walmart" && walmartNotice && <div className="message success-message">{walmartNotice}<button onClick={()=>setWalmartNotice("")}>×</button></div>}
      ...
      {tab === "walmart" && walmartBusy && !walmartReturns.skus.length && <div className="dashboard-skeleton"><i/><i/><i/><i/><i/><i/></div>}
      {tab === "walmart" && !walmartError && (!walmartBusy||walmartReturns.skus.length>0) && <ReturnsView key={`walmart-${from}-${to}-${granularity}-${sku}`} data={walmartReturns} previous={emptyReturns} selectedSku={sku} onSkuSelect={setSku} platform="Walmart Marketplace" />}
      ...
      {tab === "tts" && ttsTab === "health" && <SyncView data={sync} admin={admin} busy={busy} onSync={syncNow} onAffiliateBackfill={backfillAffiliateNow} />}
      {tab === "tts" && (ttsTab === "sales" || ttsTab === "marketing") && <LightweightView key={`${ttsTab}-${from}-${to}-${granularity}-${product}-${sku}`} type={ttsTab} pnl={pnl} returns={returns} />}
...
function GlobalFilters(props: {
  ...
  return <div className="filters global-filters">
    ...
    <label>Date To<input type="date" value={props.to} max={props.maxDate} onChange={(event) => props.setTo(event.target.value)} /></label>
    ...
    <label>Search SKU<div className="sku-search"><input list="sku-options" value={props.sku === "ALL" ? "" : props.sku} placeholder="All SKUs" onChange={(event) => props.setSku(event.target.value || "ALL")} /><button type="button" className={props.sku === "ALL" ? "selected" : ""} onClick={()=>props.setSku("ALL")}>All</button></div><datalist id="sku-options">{props.dimensions.skus.map((value) => <option key={value} value={value} />)}</datalist></label>
...
function PnlView({ data, previous, admin, onOpenCosts, onSkuSelect }: { data: PnlData; previous: PnlData; admin: boolean; onOpenCosts: () => void; onSkuSelect: (sku: string) => void }) {
  ...
  return <>
    ...
    <div className="cost-strip">{costMetrics.map(([label, value, source]) => { const unavailable = !value && (label === "Affiliate Commission" || label === "Ad Spend"); const affiliate=label==="Affiliate Commission"?data.affiliateCoverage:undefined; return <article key={label}><span>{label}<i title={source}>i</i></span><strong>{unavailable ? label === "Ad Spend" ? "Not Connected" : "Pending" : money.format(value)}</strong><small>{affiliate?`${affiliate.finalRows} Final · ${affiliate.estimatedRows} Estimated · ${affiliate.percent.toFixed(1)}% mapped`:unavailable ? label === "Ad Spend" ? "Requires TikTok Ads API or upload" : "Missing is not treated as $0" : percentOf(value, total.gmv)}</small></article>; })}</div>
    {data.affiliateCoverage && <div className={`affiliate-coverage ${data.affiliateCoverage.status}`}><strong>Affiliate Mapping</strong><span>{data.affiliateCoverage.mappedRows} / {data.affiliateCoverage.totalRows} attributed SKU rows mapped ({data.affiliateCoverage.percent.toFixed(1)}%)</span><span>Final {preciseMoney.format(data.affiliateCoverage.finalAmount)}</span><span>Estimated {preciseMoney.format(data.affiliateCoverage.estimatedAmount)}</span><em>{data.affiliateCoverage.status.replace("-"," ").toUpperCase()}</em></div>}
...
function ReturnsView({ data, previous, selectedSku, onSkuSelect, platform = "TikTok" }: { data: ReturnsData; previous: ReturnsData; selectedSku: string; onSkuSelect: (sku: string) => void; platform?: string }) {
  ...
  return <>
    ...
    {selected && <div className="dashboard-grid two-up"><section className="panel chart-panel"><div className="section-head"><div><p className="kicker">SELECTED SKU · CUSTOMER RETURN REASONS</p><h2>{`Why ${selected.sku} Is Returned`}</h2><small>{platform.startsWith("Best Buy") ? "Mirakl customer return reasons only. Refund workflow codes are excluded from categories and denominator." : `保留 ${platform} 返回的原始原因，不做人工合并。`}</small></div>{platform.startsWith("Best Buy")&&selected.reasonCoverage&&<span className="range-badge">Customer reasons {number.format(selected.reasonCoverage.mappedUnits)} / {number.format(selected.reasonCoverage.totalUnits)} · {selected.reasonCoverage.rate.toFixed(1)}%</span>}</div>{platform.startsWith("Best Buy")&&selected.reasonAudit&&selected.reasonAudit.workflowUnits>0&&<p className="reason-audit">Refund workflow excluded: {number.format(selected.reasonAudit.workflowUnits)} units · matched to an existing customer return: {number.format(selected.reasonAudit.duplicateUnits)} · customer reason unavailable: {number.format(selected.reasonAudit.unavailableReasonUnits)}</p>}<ReasonDistribution skus={data.skus} selected={selected} /></section><section className="panel chart-panel"><div className="section-head"><div><p className="kicker">SKU × CUSTOMER RETURN REASON MIX (100%)</p><h2>Customer Reason Composition by SKU</h2></div></div><ReasonDistribution skus={data.skus} /></section></div>}
...
function ReasonDistribution({ skus, selected }: { skus: ReturnSku[]; selected?: ReturnSku }) {
  ...
  if (selected) { const byReason = new Map(selected.reasons.map((reason) => [reason.reason, reason])); const max=Math.max(...selected.reasons.map(r=>r.count),1); return <><div className="metric-switch"><button className={mode==='share'?'active':''} onClick={()=>setMode('share')}>% Composition</button><button className={mode==='units'?'active':''} onClick={()=>setMode('units')}>Return Units</button></div><div className="reason-ranked">{universe.map((name) => { const reason = byReason.get(name) || { reason: name, count: 0, share: 0, refundAmount: 0,rawReasons:[] }; const width=mode==='share'?reason.share:reason.count/max*100; return <button className="tip" style={{display:"grid",gridTemplateColumns:"minmax(180px,270px) minmax(220px,1fr) 78px",gap:14,alignItems:"center",width:"100%"}} data-tooltip={`${reason.reason}\nReturned Units: ${reason.count}\nShare of SKU Returns: ${reason.share.toFixed(2)}%\nRefund Amount: ${preciseMoney.format(reason.refundAmount)}\nSource: TikTok Returns API`} key={reason.reason}><strong style={{whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{reason.reason}</strong><i style={{display:"block",width:"100%",height:16,background:"#edf3fa",borderRadius:999,overflow:"hidden"}}><u style={{ display:"block",height:"100%",borderRadius:999,width: `${width}%` }} /></i><em style={{textAlign:"right",whiteSpace:"nowrap"}}>{mode==='share'?`${reason.share.toFixed(1)}%`:`${reason.count} units`}</em></button>; })}</div></>; }
...
function SyncView({ data, admin, busy, onSync, onAffiliateBackfill }: { data: SyncData; admin: boolean; busy: boolean; onSync: () => void; onAffiliateBackfill: () => void }) {
  const last = data.lastRun; const counts = data.counts;
  return <section className="sync-layout"><article className="panel sync-hero"><p className="kicker">TIKTOK SHOP OPEN API</p><h2>{data.configured ? "自动数据管道已就绪" : "还差 TikTok API 凭证"}</h2><p>{data.configured ? "Cloudflare Worker 每 6 小时拉取订单、退货、Affiliate attribution 和 SKU 级财务结算，并写入 D1。" : "在 Cloudflare Secrets 中加入 App Key、App Secret、Access Token 与 Shop Cipher 后即可启用。"}</p>{admin && <div className="sync-actions"><button className="primary-action" onClick={onSync} disabled={busy || !data.configured}>{busy ? "同步中…" : "立即同步最近 30 天"}</button><button className="primary-action secondary" onClick={onAffiliateBackfill} disabled={busy || !data.configured}>回填 Affiliate · Feb 2026 → Today</button></div>}</article><article className="panel sync-details"><div><span>Orders API</span><b>POST · 202309</b></div><div><span>Returns API</span><b>POST · 202602</b></div><div><span>Affiliate Seller</span><b>POST · 202410</b></div><div><span>Finance API</span><b>GET · 202501</b></div><div><span>Cloudflare Cron</span><b>Every 6 hours</b></div>{counts && <><div><span>D1 Sales Lines</span><b>{number.format(counts.salesLines)}</b></div><div><span>D1 Return Lines</span><b>{number.format(counts.returnLines)}</b></div></>}</article><article className="panel sync-run"><p className="kicker">LAST RUN</p>{last ? <><h2 className={last.status === "success" ? "green" : last.status === "failed" ? "red" : ""}>{last.status.toUpperCase()}</h2><p>{last.message}</p><div><span>销售行</span><b>{last.ordersUpserted}</b></div><div><span>退货行</span><b>{last.returnsUpserted}</b></div><small>{new Date(last.startedAt).toLocaleString("zh-CN")}</small></> : <Empty>尚未执行过 TikTok 同步。</Empty>}</article>{counts && <><article className="panel sync-run"><p className="kicker">AFFILIATE COVERAGE</p><div><span>Attributed SKU Rows</span><b>{counts.affiliateAttributions}</b></div><div><span>Commission Mapped</span><b>{counts.affiliateMapped}</b></div><div><span>Final</span><b>{counts.affiliateFinal}</b></div><div><span>Estimated</span><b>{counts.affiliateEstimated}</b></div></article><article className="panel sync-run"><p className="kicker">COST RULE COVERAGE</p><div><span>Product Cost</span><b>{counts.productCostRules}</b></div><div><span>Agency Rules</span><b>{counts.agencyRules}</b></div><div><span>Return Shipping</span><b>{counts.returnShippingRules}</b></div><div><span>Manual Costs</span><b>{counts.manualCosts}</b></div></article></>}</section>;
}

function LightweightView({ type, pnl, returns }: { type: "sales" | "marketing"; pnl: PnlData; returns: ReturnsData }) {
  if (type === "sales") return <><section className="panel lightweight"><p className="kicker">SALES & ORDERS</p><h2>订单活动概览</h2><div className="kpi-grid sales-order-kpis"><Kpi label="TRUE SALES ORDERS" value={number.format(pnl.orderMix?.commercialOrders ?? pnl.total.orders)} note={`${number.format(pnl.orderMix?.commercialUnits ?? pnl.total.units)} units · samples excluded`} status="Actual" /><Kpi label="SAMPLE ORDERS" value={pnl.orderMix ? number.format(pnl.orderMix.sampleOrders) : "Not Mapped"} note={`${number.format(pnl.orderMix?.sampleUnits || 0)} sample units`} status={pnl.orderMix ? "Actual" : "Not Mapped"} /><Kpi label="RETURN ORDERS" value={number.format(pnl.orderMix?.returnedOrders ?? returns.returnedUnits)} note={`${number.format(returns.returnedUnits)} returned units`} status="Actual" /><Kpi label="GMV" value={money.format(pnl.total.gmv)} note="TikTok Orders API" status="Actual" /></div><TrendChart rows={pnl.trend} /></section></>;
  return <><section className="panel lightweight"><p className="kicker">MARKETING</p><h2>Marketing Cost Overview</h2><p>Affiliate Commission 使用 Shop Finance API；Agency Cost 使用已上传的 Cost Rules。缺失数据不会显示成 $0。</p><MarketingChart rows={pnl.trend} /></section><section className="panel campaign-panel"><div className="section-head"><div><p className="kicker">SELLER CENTER / MARKETING / CAMPAIGNS</p><h2>TikTok Shop Platform Campaigns</h2><small>Back to School / Premium Offers / platform Flash Sale registration</small></div><span className="source-state not-connected">Platform Campaign API Not Available</span></div><div className="campaign-columns"><span>Campaign</span><span>Start</span><span>End</span><span>Registration</span><span>Registered Products</span><span>Campaign Type</span></div><Empty>当前 TikTok Shop Seller Open API 未提供平台 Campaign 列表、报名状态或已报名商品 endpoint。此处不会使用 Ads API 或卖家自建 Promotion activities 代替，也不会显示伪造的空 Campaign 数据。</Empty></section></>;
}
