...
function CostDonut({ total }: { total: PnlRow }) {
  ...
  return <div className="cost-donut-card"><div className="return-donut tip" data-tooltip={parts.map((part) => `${part.label}: ${preciseMoney.format(part.value)} · ${sum ? (part.value / sum * 100).toFixed(2) : 0}%`).join("\n")} style={{ background: `conic-gradient(${gradient || "#e6eef8 0deg 360deg"})` }}><span><b>{money.format(sum)}</b><small>modeled cost</small></span></div><div className="donut-legend">{parts.map((part, index) => <div className="tip" data-tooltip={`${part.label}\n${preciseMoney.format(part.value)}\n${sum ? (part.value / sum * 100).toFixed(2) : 0}% of cost`} key={part.label}><i style={{ background: palette[index] }} /><span>{part.label}</span><b>{sum ? (part.value / sum * 100).toFixed(1) : 0}%</b></div>)}</div></div>;
}
