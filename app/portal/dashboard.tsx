...
export default function Dashboard({ email, admin, initialAllowed }: { email: string; admin: boolean; initialAllowed: Allowed[] }) {
  ...
  async function configureWalmartNow(){const clientId=prompt("Walmart Client ID")||"",clientSecret=prompt("Walmart Client Secret")||"";if(!clientId||!clientSecret)return;setWalmartBusy(true);try{await getJson("/api/walmart/credential",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({clientId,clientSecret})});setWalmartNotice("Walmart API credentials encrypted and saved.");}catch(cause){setWalmartError(cause instanceof Error?cause.message:"Configuration failed");}finally{setWalmartBusy(false);}}
  ...
  return <main className="shell">
    ...
    <section className="workspace">
      ...
      {tab === "walmart" && <><div className="subnav tts-subnav"><button disabled>P&L · Next</button><button className="active">R&R</button>{admin&&<button onClick={()=>void configureWalmartNow()} disabled={walmartBusy}>Configure API</button>}<button onClick={()=>void syncWalmartNow()} disabled={walmartBusy||!admin}>{walmartBusy?"Syncing…":"Sync Walmart"}</button></div><GlobalFilters from={from} to={to} setFrom={setFrom} setTo={setTo} granularity={granularity} setGranularity={setGranularity} product={product} setProduct={setProduct} sku={sku} setSku={setSku} dimensions={walmartReturns.dimensions} returnsMode={false} returnType={returnType} setReturnType={setReturnType} returnStatus={returnStatus} setReturnStatus={setReturnStatus} busy={walmartBusy} refresh={refreshWalmart}/></>}
