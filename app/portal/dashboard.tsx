...
function LightweightView({ type, pnl, returns }: { type: "sales" | "marketing"; pnl: PnlData; returns: ReturnsData }) {
  ...
  return <><section className="panel lightweight"><p className="kicker">MARKETING</p><h2>Marketing Cost Overview</h2><p>Affiliate 与 Agency 使用现有 Finance / Cost Rules；Ads API 未连接时不显示为 $0。</p><MarketingChart rows={pnl.trend} /></section><section className="panel campaign-panel"><div className="section-head"><div><p className="kicker">CAMPAIGN PERFORMANCE</p><h2>Campaign Timeline & Spend</h2><small>Campaign name / start / end / status / reported spend / daily average</small></div><span className="source-state not-connected">Not Connected</span></div><div className="campaign-columns"><span>Campaign</span><span>Start</span><span>End</span><span>Status</span><span>Reported Spend</span><span>Avg / Day</span></div><Empty>需要 TikTok Ads/Marketing API 的 Advertiser ID 与 Ads Access Token。Shop Open API 凭证无法读取 Campaign 报表。</Empty></section></>;
}
