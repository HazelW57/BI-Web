...
export default function Dashboard({ email, admin, initialAllowed }: { email: string; admin: boolean; initialAllowed: Allowed[] }) {
  ...
  async function syncBbyNow(){setBusy(true);setError("");try{let cursor=new Date(`${from}T00:00:00Z`),end=new Date(`${to}T00:00:00Z`);let orders=0,returned=0,windows=0;while(cursor<=end){const a=dateInput(cursor);setNotice(`Best Buy ${bbyStore} 同步中：${a}（每日安全窗口）`);const result=await getJson<{orders:number;returns:number}>("/api/bby/sync",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({store:bbyStore,from:a,to:a})});orders+=result.orders;returned+=result.returns;windows++;cursor.setUTCDate(cursor.getUTCDate()+1);}setNotice(`Best Buy ${bbyStore} 同步完成：${windows} 个窗口，${orders} 条销售行，${returned} 条退款行。`);await refreshBby();}catch(cause){setError(cause instanceof Error?cause.message:"Best Buy 同步失败");setBusy(false);}}
