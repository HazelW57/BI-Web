...
function AccessView({ admin, items, newEmail, setNewEmail, update }: { admin: boolean; items: Allowed[]; newEmail: string; setNewEmail: (value: string) => void; update: (method: "POST" | "DELETE", target: string) => void }) {
  return <section className="panel access"><div className="section-head"><div><p className="kicker">ACCESS CONTROL</p><h2>允许访问的邮箱</h2></div><span className="owner">最终管理员 · Hazel</span></div>{!admin ? <div className="empty"><strong>只有 {ADMIN} 可以管理访问权限。</strong></div> : <><form className="invite" onSubmit={(event) => { event.preventDefault(); void update("POST", newEmail); }}><label>允许新的邮箱访问<small>添加后，请把临时密码安全地发给对方。</small></label><input type="email" required value={newEmail} onChange={(event) => setNewEmail(event.target.value)} placeholder="colleague@saphiant.com" /><button>加入白名单</button></form><div className="allowlist"><div><span>邮箱</span><span>权限</span><span>状态</span><span /></div>{items.map((item) => <div key={item.email}><span>{item.email}</span><span>{item.email === ADMIN ? "超级管理员" : "查看者"}</span><span className="green">● 已允许</span><button disabled={item.email === ADMIN} onClick={() => void update("DELETE", item.email)}>{item.email === ADMIN ? "不可删除" : "移除"}</button></div>)}</div></>}</section>;
}
