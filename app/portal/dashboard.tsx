...
export default function Dashboard({ email, admin, initialAllowed }: { email: string; admin: boolean; initialAllowed: Allowed[] }) {
  ...
  const refreshWalmart=useCallback(async()=>{setWalmartBusy(true);setWalmartError("");try{const q=new URLSearchParams({from,to,granularity,sku});setWalmartReturns(await getJson<ReturnsData>(`/api/walmart/returns?${q}`));}catch(cause){setWalmartError(cause instanceof Error?cause.message:"Walmart data unavailable");}finally{setWalmartBusy(false);}},[from,to,granularity,sku,getJson]);
  ...
  async function syncWalmartNow(){setWalmartBusy(true);setWalmartError("");try{let cursor=new Date(`${from}T00:00:00Z`),end=new Date(`${to}T00:00:00Z`),orders=0,returned=0;while(cursor<=end){const a=dateInput(cursor),monthEnd=new Date(Date.UTC(cursor.getUTCFullYear(),cursor.getUTCMonth()+1,0)),b=dateInput(monthEnd>end?end:monthEnd);setWalmartNotice(`Walmart syncing ${a} — ${b}`);const result=await getJson<{orders:number;returns:number}>("/api/walmart/sync",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({from:a,to:b})});orders+=result.orders;returned+=result.returns;cursor=new Date(Date.UTC(cursor.getUTCFullYear(),cursor.getUTCMonth()+1,1));}setWalmartNotice(`Walmart sync complete: ${orders} order lines, ${returned} return lines.`);await refreshWalmart();}catch(cause){setWalmartError(cause instanceof Error?cause.message:"Walmart sync failed");}finally{setWalmartBusy(false);}}
