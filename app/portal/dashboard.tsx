...
export default function Dashboard({ email, admin, initialAllowed }: { email: string; admin: boolean; initialAllowed: Allowed[] }) {
  ...
  async function syncBbyNow(){setBusy(true);setError("");try{let cursor=new Date(`${from}T00:00:00Z`),end=new Date(`${to}T00:00:00Z`);let orders=0,returned=0,windows=0;while(cursor<=end){let next=new Date(cursor);next.setUTCDate(next.getUTCDate()+6);if(next>end)next=end;const a=dateInput(cursor),z=dateInput(next);setNotice(`Best Buy ${bbyStore} 同步中：${a} → ${z}`);const result=await getJson<{orders:number;returns:number}>("/api/bby/sync",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({store:bbyStore,from:a,to:z})});orders+=result.orders;returned+=result.returns;windows++;cursor=new Date(next);cursor.setUTCDate(cursor.getUTCDate()+1);}setNotice(`Best Buy ${bbyStore} 同步完成：${windows} 个窗口，${orders} 条销售行，${returned} 条退款行。`);await refreshBby();}catch(cause){setError(cause instanceof Error?cause.message:"Best Buy 同步失败");setBusy(false);}}
